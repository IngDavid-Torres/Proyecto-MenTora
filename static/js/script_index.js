...
<p style="margin-top: 20px; font-size: 0.9em; color: #555;">
   ...
   function openModal(type) {
    // Guardamos la posición EXACTA antes de abrir
   ...
   function cerrarModal() {
    ...
    // Volvemos a permitir scroll en la página
    document.body.style.overflow = "auto";

    modal.style.display = "none";
    overlay.style.display = "none";

    //  Regresar EXACTAMENTE al punto donde el usuario estaba
    setTimeout(() => {
        window.scrollTo({
            top: lastScroll,
            behavior: "instant"   // No animado, sin saltos
        });
    }, 1);
   }
   ...
   document.addEventListener('DOMContentLoaded', function() {
    ...
    if (hamburger && navLinks) {
        ...
        console.log('ERROR: No se encontraron los elementos de navegación');
    }
   });

   // ========== CAROUSEL DE PREVIEW DE PLATAFORMA ==========
let previewIndex = 0;
let previewAutoplay;
...
// Ajustar posiciones en resize para mantener la vista correcta
...
document.addEventListener('keydown', (e) => {
    ...
    // Solo funciona si el carousel está en viewport
